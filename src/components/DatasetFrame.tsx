...
const valenceData = [
  { name: 'Positive', valenceValue: 87.9, color: '#5D866C' },
  { name: 'Negative', valenceValue: 12.1, color: '#C96E58' }
];

const arousalData = [
  { name: 'Low Arousal', arousalValue: 76.4, color: '#5D866C' },
  { name: 'High Arousal', arousalValue: 23.6, color: '#C96E58' }
];
    ...
    >
      <div className="pudding-container">
        ...
        <div className="space-y-4">
          ...
          <div style={{ height: '400px', marginBottom: '1rem', minHeight: '400px', minWidth: 0 }}>
              ...
              >
                ...
                >
                  {emotionDistributionData.map((entry, index) => (
                    <Cell key={`chart-emotion-dist-cell-${index}`} fill={entry.color} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <p style={{ color: 'var(--text-color)' }}>
            ...
            {' '}to confirm the distribution wasn't random.
          </p>
          ...
          <p style={{ color: 'var(--text-color)' }}>
            To put that in perspective, a value above 20 is usually considered significant. At the scale of 132K, chi-square test will return a significant p-value for almost any non-uniform distribution (the law of large numbers). But we still get the key insight: the issue is systematic.
          </p>
              ...
              <div style={{ height: '220px' }}>
                <ResponsiveContainer width="100%" height={220} minWidth={1}>
                  ...
                  >
                    ...
                    <Bar
                      key="valence-bar"
                      dataKey="valenceValue"
                      name="Valence"
                      radius={[4, 4, 0, 0]}
                      isAnimationActive={false}
                    >
                      {valenceData.map((entry, index) => (
                        <Cell key={`valence-cell-${index}`} fill={entry.color} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              ...
              <div style={{ height: '220px' }}>
                <ResponsiveContainer width="100%" height={220} minWidth={1}>
                  ...
                  >
                    ...
                    <Bar
                      key="arousal-bar"
                      dataKey="arousalValue"
                      name="Arousal"
                      radius={[4, 4, 0, 0]}
                      isAnimationActive={false}
                    >
                      {arousalData.map((entry, index) => (
                        <Cell key={`arousal-cell-${index}`} fill={entry.color} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <p style={{ color: 'var(--text-color)' }}>
            We don't know, and cannot say anything about misclassifying{' '}
            <span
              style={{
                position: 'relative',
                cursor: 'default',
            ...
            >
              ...
              <span
                ...
                className="group-hover:opacity-100"
              >
                We also do not really have ground truth, we have my opinion and a few friends
              </span>
            </span>
            , I do, however, have a feeling that, in ambiguous cases, "calm" works as the most common prediction.
          </p>
